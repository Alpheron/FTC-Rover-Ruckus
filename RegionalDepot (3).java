...
@Autonomous(name="NewDepot", group="Concept")
//@Disabled
public class RegionalDepot extends LinearOpMode
{
    ...
    DcMotor                 hanging1;
    DcMotor                 hanging2;
    DcMotor                 directmotor1;
    DcMotor                 directmotor2;
    CRServo                 slider1;
    CRServo                 slider2;
    CRServo                 arm2;
    CRServo                 intake1;
    ...
    public void runOpMode() 
    {
        ...
        waitForStart();
        leftPath();
        telemetry.addLine("DhruvBot initialized");
        telemetry.update();
        if (opModeIsActive()) {
        ...
        runtime.reset();
         ...
         if(position!=-1 || runtime.time() >= 10.0){
            break;
     ...
     //*******************************************************
      ...
      if (direction == 'f' || direction == 'b'){
        ...
        //   telemetry.addData("The number of rotations are", rotation);
          ...
          double strafeDist = 13.75; //1 rotation in inches
    ...
    private void moveBackward(int target){
        ...
        while (opModeIsActive() && (Math.abs(currentpos-target)>TOLERANCE)) //something is wrong with this while loop, it keeps going forever
        {
            ...
            if (Math.abs(currentpos)>=target-breaking) {
                break;
            }
    }
    stopMotor();
    ...
    private void unhang() {
        hanging1.setPower(-0.9);
        hanging2.setPower(-0.9);
        sleep(3900);
        stopMotor();
        TurnFromCurrentPos();
    ...
    private void start_tele(){
     ...
     private void unhook() {
        unhang();
        DistanceFormula(0.75, 0.75, 0.5, 1, 'r', 's');
        DistanceFormula(0.5, 1, 0.8, 1.2, 'f', 'n');
        DistanceFormula(0.8, 1.2, 1, 1, 'l','s');
    ...
    private void defaultPath() {
        ...
        rotate(45, power);
        DistanceFormula(1.2, 1.2, 0.5, 2.5, 'f', 'n');
        rotate(135, power);
        DistanceFormula(0.5, 2.5, 2.5, 2.5, 'f', 'n');
        dropmarker();
        hail_mary();
    }
    ...
    private void centerPath(){
        unhook();
        DistanceFormula(1, 1, 2.3, 2.3, 'f', 'n');
        rotate(-45, power);
        dropmarker();
        DistanceFormula(2.5, 2.5, 2.5, 2.65, 'r', '*');
		telemetry.addData("1 imu heading", lastAngles.firstAngle);
        telemetry.addData("2 global heading", globalAngle);
		telemetry.update();
		sleep(10000);
...
//         extend_arm();
    ...
    private void rightPath(){
        unhook();
        DistanceFormula(1, 1, 1.1, 1.1, 'f', '*');
        DistanceFormula(1.2, 1.2, 1.5, 0.75, 'r', 's');
        DistanceFormula(1.5, 0.75, 2.3, 1.8, 'f', 'n');
        rotate(-45, power);
        DistanceFormula(2.5, 1.8, 2.5, 2.5, 'l', '*');
        dropmarker();
        hail_mary();
    }
    ...
    private void leftPath(){
        unhook();
        DistanceFormula(1, 1, 1.1, 1.1, 'f', '*');
        DistanceFormula(1.1, 1.1, 0.6, 0.6, 'l', '*');
        DistanceFormula(0.95, 0.8, 1.5, 2, 'f', 'n');
        rotate(-45, power);
        DistanceFormula(1.5, 2.5, 2, 2.5, 'f', 'n');
        dropmarker();
        DistanceFormula(2, 2.5, 2, 2.55, 'r', '*');
        hail_mary();
 }
    ...
    private void extend_arm(){
        directmotor1.setPower(.75);
        directmotor2.setPower(-.75);
        sleep(800);
...
//         slider2.setPower(0.0);
    ...
    private void init_PID(){
        telemetry.addData("OpMode", "Ready for Execution");
        telemetry.update();
        sleep(1000);
    ...
    private void rotate(int degrees, double power)
    {
        ...
        pidRotate.setOutputRange(.29, power);
