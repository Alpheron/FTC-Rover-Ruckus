...
//package org.firstinspires.ftc.teamcode.PIDDriveClass.java
...
@Autonomous(name="PID", group="Concept")
//@Disabled
public class PIDGyro extends LinearOpMode
{
    ...
    double                  globalAngle, power = .50, correction, TOLERANCE = 3;
	...
	private static void DistanceFormula(double x1, double y1, double x2, double y2, char direction) {
     //*******************************************************
      ...
      if (x1 < x2) {
        ...
        if (y1 < y2) {
          ...
          turn_angle = angle_b -180;
        }
      }
      //Printing all statements
      telemetry.addData(distance);
      telemetry.addData(turn_angle);
	  if (direction == 'f' || direction == 'b'){
		  int target1 = Math.rint((distance/4PI)*1120);
		  forward_backward(direction, target1);
		  }
	  else if (direction == 'l' || direction == 'r') {
		  double strafeDist = 7 //1 rotation
		  int target2 = (distance/7)*1120;
	...
	private void moveForward(int target) {
        ...
        frontleftmotor.setMode(DcMotor.RunMode.USING_ENCODER);
        backleftmotor.setMode(DcMotor.RunMode.USING_ENCODER);
		
		while (abs(frontrightmotor.getCurrentPosition()-target)<TOLERANCE && abs(frontleftmotor.getCurrentPosition()-target)<TOLERANCE && abs(backrightmotor.getCurrentPosition()-target)<TOLERANCE && abs(backleftmotor.getCurrentPosition()-target)<TOLERANCE)
        {
	...
	private void forward_backward(char direction, int target) {
		if (direction == 'f') {
			rotate(turn_angle, 0.4);
			moveForward(target);
			stopMotor();}
